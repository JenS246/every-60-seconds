...
const metrics: Metric[] = [
  ...
  {
    id: "linkedin-applications",
    category: "social",
    eyebrow: "LinkedIn",
    title: "LinkedIn applications",
    prompt: "Job applications submitted",
    value: 11_000,
    suffix: " applications",
    fact: "More than 183 job applications arrive every second.",
    source: "LinkedIn Talent Solutions",
    sourceUrl: "https://business.linkedin.com/hire/product-update/wave2-2025",
    basis: "LinkedIn reports that 11,000 members apply for jobs on the platform every minute. This is a directly published per-minute figure, so no time conversion is needed.",
    sourceDate: "2025 LinkedIn data",
    Icon: LinkedinLogo,
    accent: "#77b8ff",
    ink: "#082b55",
  },
  ...
  },
  {
    ...
    eyebrow: "TikTok",
    title: "TikTok videos",
    prompt: "New short videos uploaded",
    value: 16_000,
    suffix: " videos",
    fact: "Creators upload about 267 TikTok videos every second.",
    source: "Domo Data Never Sleeps 12.0",
    sourceUrl: "https://www.domo.com/learn/infographic/data-never-sleeps-12",
    sourceLinks: [
      { label: "Domo report", url: "https://www.domo.com/learn/infographic/data-never-sleeps-12" },
    ...
    basis: "Domo's 2024 secondary compilation reports 16,000 TikTok video uploads per minute. Domo identifies its overall source pool but does not map this individual figure to a specific underlying dataset.",
    sourceDate: "December 2024; secondary estimate",
    Icon: TiktokLogo,
    accent: "#72eadc",
    ink: "#063b36",
  ...
  },
  {
    ...
    prompt: "Short videos watched",
    value: 138_888_889,
    suffix: " views",
    fact: "That is more than 2.3 million Shorts views every second.",
    source: "YouTube Press",
    sourceUrl: "https://blog.youtube/press/",
    basis: "YouTube reports more than 200 billion Shorts views per day. Dividing that conservative floor by 1,440 minutes gives this rate.",
  ...
  },
  {
    ...
    title: "Reels reshares",
    prompt: "Reels sent across Meta apps",
    value: 3_125_000,
    suffix: " reshares",
    fact: "People reshare more than 52,000 Reels every second.",
    source: "Meta Newsroom",
    sourceUrl: "https://about.fb.com/news/2025/09/in-india-instagram-debuts-a-reels-first-experience-for-its-mobile-app/",
    basis: "Meta reports more than 4.5 billion Reels reshares per day across its platforms. That conservative floor is divided by 1,440 minutes.",
  ...
  {
    id: "netflix-watch-time",
    category: "streaming",
    eyebrow: "Netflix",
    title: "Netflix watch time",
    prompt: "Hours of shows and films streamed",
    value: 363_394,
    suffix: " hours watched",
    fact: "One minute holds more than 41 years of Netflix watch time.",
    source: "Netflix engagement reports",
    sourceUrl: "https://about.netflix.com/en/news/what-we-watched-the-second-half-of-2025",
    sourceLinks: [
      { label: "Netflix first-half report", url: "https://about.netflix.com/en/news/what-we-watched-the-first-half-of-2025" },
      { label: "Netflix second-half report", url: "https://about.netflix.com/en/news/what-we-watched-the-second-half-of-2025" },
    ],
    basis: "Netflix reports more than 95 billion hours watched in the first half of 2025 and 96 billion in the second half. The combined conservative floor of 191 billion hours is divided across 2025's 525,600 minutes.",
    sourceDate: "2025 viewing; reported July 2025 and January 2026",
    Icon: MonitorPlay,
    accent: "#d8a0ff",
    ink: "#321046",
    estimated: true,
  ...
  },
  {
    ...
    category: "downloads",
    eyebrow: "Zoom",
    title: "Zoom downloads",
    prompt: "Zoom apps installed",
    value: 288,
    suffix: " downloads",
    fact: "That is almost five new Zoom installs every second.",
    source: "Domo Data Never Sleeps 12.0",
    sourceUrl: "https://www.domo.com/learn/infographic/data-never-sleeps-12",
    sourceLinks: [
      { label: "Domo report", url: "https://www.domo.com/learn/infographic/data-never-sleeps-12" },
    ...
    basis: "Domo's 2024 graphic reports 288 Zoom app downloads per minute. It does not report Zoom calls, and Zoom does not publish a recent global call total, so this tile keeps the activity named exactly as the source measures it.",
    sourceDate: "December 2024; secondary estimate",
    Icon: VideoCamera,
    accent: "#83b7ff",
    ink: "#092b58",
...
const categories: Category[] = [
  ...
  {
    id: "streaming",
    eyebrow: "Video activity",
    title: "Watching & sharing",
    description: "Views, streams, and reshares moving through video platforms.",
    accent: "#d0a0ff",
    ink: "#301047",
    Icon: MonitorPlay,
  },
...
function buildChallengeMetrics() {
  const categoryPicks = shuffled(categories).map((category) => {
    const categoryMetrics = metrics.filter((metric) => metric.category === category.id);
    return shuffled(categoryMetrics)[0];
  });
  const pickedIds = new Set(categoryPicks.map((metric) => metric.id));
  const remaining = shuffled(metrics.filter((metric) => !pickedIds.has(metric.id)));
  return shuffled([...categoryPicks, ...remaining.slice(0, challengeSize - categoryPicks.length)]);
}

function formatValue(metric: Metric, value = metric.value) {
  if (metric.value < 1_000) return Number.isInteger(metric.value) ? Math.round(value).toLocaleString("en-US") : value.toFixed(2);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="metric-front" aria-hidden={revealed}>
      ...
      <section className="hero" id="top">
        <div className="hero-copy">
          ...
          <div className="hero-actions">
            ...
            <button className="explore-button" onClick={exploreAll}>Explore all data</button>
          </div>
        </div>

        <div className={`timer-orbit ${status}`} aria-hidden="true">
      ...
      <section className="game-section" aria-labelledby="game-title" id="explore" ref={explorerRef}>
        <div className="section-heading">
          <div>
            ...
            <h2 id="game-title">What happens in a minute?</h2>
          </div>
          <div className="score-pill">
            <strong>{explorerRevealed.size}</strong> / {totalMetrics} revealed
          </div>
        </div>

        <div className="overview-wrap">
          <MetricCard metric={overviewMetric} revealed={explorerRevealed.has(overviewMetric.id)} onReveal={revealExplorerMetric} featured />
        </div>
        ...
        <div className="category-list">
          {categories.map((category) => {
            ...
            const revealedInCategory = children.filter((metric) => explorerRevealed.has(metric.id)).length;
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div className="category-body" id={`category-${category.id}`} hidden={!isOpen}>
                    <div className="metric-grid">
                      {children.map((metric) => (
                        <MetricCard key={metric.id} metric={metric} revealed={explorerRevealed.has(metric.id)} onReveal={revealExplorerMetric} />
                      ))}
                    </div>
                  </div>
              </section>
